...
export default function ImportWalletScreen() {
  ...
  const handlePaste = async () => {
    try {
      ...
      if (words.length < 12) {
        toast.error(
          `Invalid Phrase! Found only ${words.length} words. Please ensure you have 12 words.`
        );
        return;
      }
  ...
  const validateSeedPhrase = (phrase: string): boolean => {
    const words = phrase.trim().split(' ').filter(word => word.length > 0);

    // Simple validation for demo (length check + simple char check)
  ...
  const handleImportWallet = () => {
    ...
    router.push({
      pathname: '/wallet-setup/import-name-wallet',
